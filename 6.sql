...
CREATE OR REPLACE TABLE data_in AS (
  ...
  cols AS (
  	 SELECT list_slice(regexp_split_to_array(val[2:-1], ' \*| \+'), 1, -1) cols FROM raw
	 ORDER BY
	 n DESC  -- only interested in the last row containing the ops for this.
	 LIMIT 1
  ),col_lengths AS (
   	SELECT
  ...
  SELECT list(digit::string ORDER BY n) as digit FROM chopped
  GROUP BY c
);
